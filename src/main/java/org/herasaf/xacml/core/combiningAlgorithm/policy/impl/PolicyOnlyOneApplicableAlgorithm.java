...
/**
 * The implementation of the default XACML 2.0 <i>policy only one applicable algorithm</i>.<br />
 * See: <a href= "http://www.oasis-open.org/committees/tc_home.php?wg_abbrev=xacml#XACML20"> OASIS eXtensible Access
 * Control Markup Langugage (XACML) 2.0, Errata 29. January 2008</a> pages 152 - 153, for further information.
 * 
...
public class PolicyOnlyOneApplicableAlgorithm extends PolicyUnorderedCombiningAlgorithm {
    ...
    public DecisionType evaluateEvaluatableList(final RequestType request, final List<Evaluatable> possiblePolicies,
        ...
        for (int i = 0; i < possiblePolicies.size(); i++) {
            ...
            case NOT_APPLICABLE:
                ...
                if (evaluationContext.isTargetMatched()) {
                    ...
                    if (firstApplicableDecision != null) {
                        /*
                         * When indeterminate is returned, it has to be sure that the returned error is a processing
                         * exception. because of this, the evaluation context have to be reset and set to
                         * Processing-Exception. See: OASIS eXtensible Access Control Markup Langugage (XACML) 2.0,
                         * Errata 29. January 2008</a> pages 143-144 and pages 152-153 for further information.
                         */
                        evaluationContext.resetStatus();
                    ...
                    /*
                     * If no decision has been made yet, the decision has to be remembered including the status code and
            ...
            default:
                /*
                 * If no decision has been made yet, the decision has to be remembered including the status code and the
                 * missing attribute data. If there was already another applicable Evaluatable, Indeterminate has to be
                 * returned.
                 */
                if (firstApplicableDecision == null) {
                    firstApplicableDecision = decision;
                     ...
                     * When indeterminate is returned, it has to be sure that the returned error is a processing
                     * exception. because of this, the evaluation context have to be reset and set to
                     * Processing-exception. See: OASIS eXtensible Access Control Markup Langugage (XACML) 2.0, Errata
                     * 29 June 2006</a> page 86 and page 139 and the specification of the only-one-applicable algorithm
        ...
        /*
         * When the decision was made, the statuscode and the missing attributes where saved. To return the result, they
         * have to be set to the state of the decision. Or if the policy is not applicable it has to be sure that no
         * chances of the state have been made.
         */
        evaluationContext.resetStatus();
        if (firstApplicableDecision != null) {
