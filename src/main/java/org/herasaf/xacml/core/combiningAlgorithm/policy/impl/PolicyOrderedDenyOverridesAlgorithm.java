...
/**
 * The implementation of the default XACML 2.0 <i>policy ordered deny overrides algorithm</i>.<br />
 * See: <a href= "http://www.oasis-open.org/committees/tc_home.php?wg_abbrev=xacml#XACML20"> OASIS eXtensible Access
 * Control Markup Langugage (XACML) 2.0, Errata 29. January 2008</a> page 148, for further information.
 * 
...
public class PolicyOrderedDenyOverridesAlgorithm extends PolicyOrderedCombiningAlgorithm {
    ...
    public DecisionType evaluateEvaluatableList(final RequestType request, final List<Evaluatable> possiblePolicies,
        ...
        for (int i = 0; i < possiblePolicies.size(); i++) {
            ...
            switch (decision) {
            case DENY:
                if (!evaluationContext.isRespectAbandonedEvaluatables()) { // if abandoned
                    // evaluatables should
