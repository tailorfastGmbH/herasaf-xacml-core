...
/**
 * This is the very simple implementation of the {@link PolicyRepository}. This policy repository has a limited
 * functionality. It only works as a "provider" of locally deployed {@link Evaluatable}s.<br />
 * <br />
 * This implementation doesn't support all operations defined in {@link PolicyRepository}. Some methods throw a
 ...
 * </ul>
 * <b>WARNING: </b>It is not recommended to use this repository in a productive environment.<br />
 * 
...
public class MapBasedSimplePolicyRepository implements PolicyRepository {
     ...
     *            True if the root policy combining algorithm in the PDP is ordered, false otherwise.
     */
    public MapBasedSimplePolicyRepository() {
        ...
        logger.warn("This policy repository ({}) must not be used in a productive environment.", this.getClass()
                .getCanonicalName());
    }
    ...
    /**
     * Checks for consistency of the given {@link Evaluatable}s. The provided {@link Map} of {@link Evaluatable}s
     * (indexed by the {@link EvaluatableID} s). A root {@link Evaluatable} must be consistent in itself.
     * 
     * @param newIndividualEvaluatables
     *            The list of individual (meaning all sub-{@link Evaluatable}s) {@link Evaluatable}s to be checked for
     *            consistency.
     */
     ...
     * {@inheritDoc}
     * 
     * Due to the fact that this implementation does not implement any index. All root {@link Evaluatable}s are returned
     * by default.
     */
    ...
    /**
     * This is the entry point to split {@link Evaluatable}s into individuals. This means that a policy tree is split
     * into its nodes. The resulting Map contains as key all IDs that exist in this tree (of all nodes). The list
     * associated with this ID contains all {@link Evaluatable}s that have this ID and all references that point to this
     * ID. Here the policy tree is not checked for consistency. So illegal states are allowed here.
     * 
     * @param evaluatable
     ...
     * @param initialMap
     *            This map contains already {@link Evaluatable}s from a previous step. This beause this method is
     *            intended to be called recursivly down the whole policy tree.
     * @return A {@link Map} containing the whole policy tree that started at the given {@link Evaluatable} split up
    ...
    /**
     * Every {@link Evaluatable}s must be mapped to its root {@link Evaluatable} . This because in case of an
     * undeployment. All related sub- {@link Evaluatable}s must be removed from the list containing the individual
     * {@link Evaluatable}s.
     * 
     * @param id
    ...
    /**
     * Checks if a policy is unique (id) and if references are only local references.
     * 
     * @param individualEvaluatables
     *            The {@link Map} of {@link Evaluatable} to be checked for consitency.
     */
